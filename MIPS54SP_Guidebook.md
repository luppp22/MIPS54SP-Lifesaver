...
A：教材、实验指导、《自己动手写CPU》（在写代码的时候很有帮助）、MIPS指令手册、自己之前写的CPU、学长代码~~（不太推荐）~~。当然，阅读完本指南，相信你就能解决大部分问题！



Q：这次的静态流水线有什么要求？

A：主要就是通过流水线暂停处理数据冲突，通过提前分支判断和延时槽处理转移指令。
...
A：~~本人是菜鸡，不保证内容的正确性，~~同时硬件课程组的要求日新月异，本指南只能保证适合本人所在学期，因此所有内容仅供参考。如果发现真的存在问题，欢迎在Issues区提出。
